...
import * as read from "../be/db/tasks/read";
...
// Complete public facade and selected range manifests captured from merged base 8a8b7b53.
...
const selected = `type:ExistingTrackerContextWork
type:ExistingTrackerContextWorkReason
type:TaskFilters
value:findCompletedTaskInThread
value:findExistingLinearTrackerContextWork
...
value:markSlackRelayDelivered
value:markTasksNotified
value:resetTasksNotified`.split("\n");
const internal = [
  "type:AgentTaskRow",
...
test("complete facade, selected public bindings and internal-only manifest remain exact", async () => {
  ...
  const target = await Bun.file(new URL("../be/db/tasks/read.ts", import.meta.url)).text();
  ...
  for (const binding of selected) {
    ...
    if (kind === "value")
      expect((db as Record<string, unknown>)[name]).toBe((read as Record<string, unknown>)[name]);
  }
  ...
  expect(facade.split("\n").length).toBeLessThanOrEqual(15219);
});
