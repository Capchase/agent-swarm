...
- **Core tools** — 25 tools classified for immediate availability when their capabilities are enabled. These cover task lifecycle, memory, swarm awareness, and recurring operations: `script-run`, `db-query`, `kv-get`, `script-query-types`, `get-repos`, `accept-steer`, `memory-edit`, `script-search`, `steer-task`, `kv-list`, and `get-config`.
- **Deferred tools** — Discovered on demand via Claude Code's Tool Search, including scheduling, configuration writes, skills, workflows, MCP servers, Slack, and user identity.

Loading recurring operations upfront avoids repeated discovery while leaving less frequent tools out of the initial context.
...
The `kv` capability exposes four room tools for state shared with browser pages, agents, scripts, and workflows:

| Tool | Purpose |
|------|---------|
| `room-get` | Read current room state. |
| `room-change` | Apply `set`, `delete`, `insert`, `increment`, or `text` operations. |
| `room-reset` | Replace the document with a new generation. |
| `room-decode` | Decode a saved snapshot without changing the room. |

Scripts use `ctx.swarm.room.get/change/reset/decode`. Browser pages use `swarmSdk.room(name, {schemaVersion})` for state and presence, and `swarmSdk.channel(name)` for transient messages. Room changes do not create page versions.

Room snapshots have a 2 MiB encoded limit and normally flush within one second. Presence and channel messages are not persisted. A schema mismatch keeps state readable but rejects writes until an explicit reset. Deploy a single API replica for shared room state.

See the [Rooms API reference](/docs/api-reference/rooms) for request schemas.
...
| `routingReason` | string | With `agentId` | Why this agent was selected: `skill`, `continuity`, `overflow`, `human_pinned`, or `reroute_fault`. Omit for pool routing |
| `routingNote` | string | No | Optional routing context, up to 200 characters |
...
| `outputSchema` | object | No | JSON Schema the assignee's final output must satisfy. `store-progress` rejects a completion that doesn't match. Supported keywords: `type`, `required`, `properties`, `enum`, `const`, `items`; every nested schema under `properties`/`items` must itself be a plain object, or `send-task` rejects it at ingress. |
...
For a nonterminal task, progress text such as "waiting for" or "blocked on" can return `blockedWaitingElapsedMs` and a nudge to use `defer-task` when at least three minutes have elapsed since the previous update. This is advice only; it does not create a wake-up or change the task's status.
...
Complete the current task now and wake up later to continue it. Use it when the result needs time: a build, a deploy, a reply.

Pass `taskId`, one of `delayMs` or `runAt`, a `summary` of what you did and where things stand, a `note` that says what is pending, and optional `checks` to verify on wake-up. The task reaches its final state (`completed`) on this call. A one-off schedule targets you, and its child task carries the deferred task as `parentTaskId` so the wake-up run receives the prior context. Use this tool instead of creating a continuation schedule by hand.

- **Early wake-up:** optionally pass `wakeOn: { event: "settled", taskId: "<watched-task-id>" }`. Events are `task.completed`, `task.failed`, or `settled` (completed or failed). The watched task must exist, be nonterminal, and differ from the task being deferred. A `delayMs` or `runAt` deadline is still required; the event and deadline share a single wake-up so racing triggers do not create duplicate children. Pending waits survive API restarts.
- **Structured output:** if the task has an `outputSchema`, pass `output` as a JSON string matching it. The tool stores that string verbatim as terminal output and puts the summary and deferral details in the task log. Missing or invalid output is rejected before creating a schedule. Without a schema, the summary and deferral details become terminal output, and `output` is ignored.

See [deferring until another task finishes](/docs/concepts/scheduling#deferring-until-another-task-finishes) for an example.

Optional `wakeOn: { taskId, event }` wakes the continuation early on `task.completed`, `task.failed`, or `settled` (completed or failed). A `delayMs` or `runAt` ceiling is still required. If the watched task defers, the wait follows its resume task across further deferrals without changing that ceiling. Disabling or deleting a successor schedule before it runs makes the parent's completion eligible on the next reconciliation; a wait for `task.failed` still falls back to its ceiling.
...
| `contentType` | enum | Yes | `text/html` or `image/svg+xml` (served directly at `/p/:id`), or `application/json` (rendered by the SPA) |
...
Execute a read-only SQL query against the swarm database. Available to all authenticated agents — be aware results may include secrets (`oauth_tokens`, `configs`). Results are capped by the operator-configurable `DB_QUERY_MCP_MAX_ROWS` limit. Read-only enforcement also applies to fallback execution when bounded queries are disabled; write statements with `RETURNING` cannot bypass it.
...
audit trail. Agents can edit their own memories, including memories they shared
with the swarm. Editing another agent's memory by ID requires a lead agent;
shared visibility does not grant edit permission.
