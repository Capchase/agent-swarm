---
title: Salesforce
description: Salesforce MCP connector authenticated via OAuth. Lets agents access Salesforce sObjects using Salesforce's hosted MCP endpoint.
---

Agent Swarm can connect to Salesforce as an MCP server over HTTP, authenticated via OAuth. The connector gives agents access to Salesforce sObjects through Salesforce's native hosted MCP endpoint — no proxy, no middleware.

<Callout type="info">
This guide covers Salesforce's `sobject-reads` MCP endpoint, which provides SOQL read access to sObjects. Write operations use a different endpoint and are not covered here. Choose the OAuth scopes appropriate for your use case.
</Callout>
...
- **Gives agents sObject access.** Once connected, any agent with the Salesforce server in scope can run sObject queries.
...
- **Transport:** HTTP
- **URL:** `https://api.salesforce.com/platform/mcp/v1/platform/sobject-reads` (adjust the path if you are targeting a different Salesforce MCP endpoint)
