...
A ready-to-use `slack-manifest.json` is included in the repository root — import it directly in the Slack App configuration page to set up all scopes, events, and features automatically. After adding channel-management scopes or `reactions:read` to an existing app, reinstall the app to the workspace so Slack grants them to the bot token. `reactions:read` is required for the acceptance-reaction cleanup in `src/slack/ack.ts` to read the message's current reactions (`reactions.get`); an app installed before this scope was added must be reinstalled, or `reactions.get` fails with `missing_scope` and cleanup is skipped entirely for that message.
...
When a task finalizes, the bot removes the acceptance reaction it applied and adds the terminal reaction. The removal candidates are the names this API process recorded when it applied them, bound to the bot identity that applied them, and intersected with the bot's live reactions on the message (`reactions.get`). The bot never removes a reaction it did not record, and never removes a reaction another bot identity applied. That record lives in process memory only. If the API restarts between acceptance and finalization, the record is lost: finalize removes nothing, the terminal reaction is still added, and the acceptance reaction stays beside it. This is a deliberate trade, agreed with the maintainer, against removing a reaction the swarm never applied.
