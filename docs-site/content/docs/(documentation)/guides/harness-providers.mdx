...
The sync lives in [`src/commands/profile-sync.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/commands/profile-sync.ts) (`syncProfileFilesToServer`) and is called once per completed session from `checkCompletedProcesses` in `runner.ts`, **gated on `adapter.traits.hasLocalEnvironment`**:

| Provider | `hasLocalEnvironment` | Session-end sync |
|---|---|---|
| `claude`, `pi`, `codex`, `opencode` | `true` | **Yes** — runs every session end |
| `devin`, `claude-managed` | `false` | No — these have no `/workspace` FS |
