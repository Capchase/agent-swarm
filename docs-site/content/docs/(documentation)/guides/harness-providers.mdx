...
The direct Codex picker includes `gpt-6-sol` and `gpt-6-luna`, verified against the
[official Codex catalog](https://developers.openai.com/codex/models) on September 23, 2026.
Use bare model IDs with the CLI and app-server. Both models support text and image
input, tools through Responses, a 1,050,000-token context window, and 128,000 output
tokens. Their API reasoning efforts are `none`, `low`, `medium`, `high`, `xhigh`, and
`max`; the swarm displays `none` as `off`. Ultra is an orchestration mode, not a
...
Above 272K input tokens, input and cache rates double and output rates multiply by
1.5 for the full request. The snapshot preserves these tiers; the existing worker
estimate and pricing-table seeder use base rates, so long-context costs remain
estimates. Defaults, aliases, and portable tiers remain on their existing models
while account availability rolls out.

Codex access depends on account, client, and sign-in method. GPT-5.5 retires from
ChatGPT-authenticated Codex on October 14, 2026, but its API availability is
unchanged. Keep API and ChatGPT lifecycle decisions separate.

## Credential readiness recovery

After boot, a live provider change triggers a fresh credential report. A not-ready report parks the worker in `waiting_for_credentials`; the runner continues polling and rechecks credentials at most once every 30 seconds after reconciling its environment. Once a ready report succeeds, the API restores dispatch eligibility without a restart, using the same runtime identity in multi-runtime mode. Failed checks or report deliveries also retry on that interval, and only one check/report runs at a time.

Ready workers retain the five-minute Bedrock enumeration refresh. `CRED_CHECK_DISABLE=1` disables both boot checks and steady-state refreshes.

## 1. The `ProviderAdapter` contract

### Claude execution transports

`ClaudeAdapter` supports `CLAUDE_TRANSPORT=cli|sdk`, with CLI as the default.
The SDK transport uses pinned `@anthropic-ai/claude-agent-sdk` and the existing installed Claude executable.
It still starts a Claude Code child process.
Both transports share credential selection, session configuration, event normalization, summaries, costs, and context tracking.
...
Session initialization persists the selected transport in `providerMeta.transport`.

Transport selection occurs for each new session.
The agent runtime editor stores an optional agent-scoped `CLAUDE_TRANSPORT` configuration row.
An omitted value preserves that row, `null` deletes it, and `cli` or `sdk` assigns it.
The worker independently rejects an effective bridge under SDK selection.
It preserves Swarm context preambles instead of native SDK resume.
...
| [`src/providers/acp-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/acp-adapter.ts) | `Bun.spawn` of the selected OpenCode preset or custom target, driven over stdio with `@agentclientprotocol/sdk`'s ndjson `ClientSideConnection` | The ACP target owns its model auth and receives a session-scoped `aseph_` token as the swarm MCP bearer |
| [`src/providers/dsh-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/dsh-adapter.ts) | Preinstalled DeepSeek Harness CLI, headless JSON over stdin/stdout | `OPENROUTER_API_KEY` for `openrouter/` models; `DEEPSEEK_API_KEY` for native IDs. No MCP, steering, resume, or cost/context telemetry |

ACP session tokens identify the session's agent and task. On the HTTP config routes, non-lead session tokens receive masked secrets and cannot write or delete config; changing `X-Agent-ID` cannot grant lead permissions. The secret scrubber redacts `aseph_` tokens in logs. Session completion starts revocation without waiting for its response. Each revoke request has a five-second timeout and logs HTTP, network, or timeout failures. If revocation fails, the token remains valid until its 24-hour expiry.
...
After a prompt completes, ACP persists a bounded, scrubbed `acp_prompt_response` raw-log diagnostic with the optional usage and `_meta` fields. Reported input/output/cache counters feed cost telemetry; missing counters are `undefined` at the adapter boundary but become zero in the session-cost API and DB. Persisted costs and UI displays cannot distinguish missing usage from measured zero; the raw diagnostic retains absence. Unresolved pricing stays `unpriced`. See [cost and context computation](/docs/guides/cost-and-context-computation#acp-prompt-usage).
...
The default-on `TASK_TOOL_PRELOAD_ENABLED` feature lets a new MCP session preload selected
swarm tools by task type or schedule. The API adds Claude's per-tool `anthropic/alwaysLoad`
metadata and a registered MCP instruction to call already-available tools directly. This does
not change the provider adapter contract, tool permissions, or other harnesses' loading policy.
See [configuration and limits](/docs/ui/configuration#task-tool-preloading-proposal).
...
### pi-mono + Amazon Bedrock auth (alpha)
...
Set `BEDROCK_AUTH_MODE=bearer` to authenticate with a Bedrock API key in `AWS_BEARER_TOKEN_BEDROCK`. The worker reports that token as missing when it is unset, even if another provider API key is configured. Both Bedrock modes require `AWS_REGION` and run the same live model enumeration probe.
...
When either Bedrock mode is active, the worker runs a **real** `ListFoundationModels` call via `@aws-sdk/client-bedrock` (dynamically imported — the API binary never loads the SDK):

- **Success** → `ready: true`, with `satisfiedBy: "sdk-delegated"` in SDK mode or `"env"` in bearer mode. The worker proceeds to claim tasks.
...
In bearer mode, supply `AWS_BEARER_TOKEN_BEDROCK`. In SDK mode, use any credential source the AWS SDK accepts:
...
| `AWS_BEARER_TOKEN_BEDROCK` | Bedrock API key | **required in bearer mode** |
...
- **Pause → resume**: a paused task restarts with a **new** session, re-applying `task.progress`, attachment fetch recipes, and the required `outputSchema` via `buildResumePrompt`; if the task is also part of a parent chain, the context-preamble path is applied before execution resumes.
...
The shared `system.agent.communication` template sets reply defaults for every session composite:

- Simple questions and action confirmations use one to three sentences. Routine replies stay under 120 words.
- Requested depth, requester preferences, required formats, and essential evidence can require longer replies. Internal investigation and required task artifacts retain their necessary detail.
- The `comms` skill handles requested simplification, re-expression, and clarity rewrites when available. Ordinary replies do not require a skill invocation.
- Visual mode requires an explicit visual request and delivery tools that work in the active channel. Requests to show data remain data requests.
- Output schemas and channel delivery rules take precedence over skill defaults. Slack's engine continues to deliver task results.

Customized prompt templates can override these defaults. The seeder preserves operator edits.
...
Follow-up and resumed task prompts keep the skill invocation on the first line, with the bounded prior-context block after it. Persisted profile-sync warnings also follow the invocation and stay ahead of prior context. Claude and the OpenCode ACP target expand the leading command natively; Codex and OpenCode use the adapter resolver. Prepending context before the command leaves it as plain text.
...
3. Uploads each remaining `plugin/commands/*.md` skill plus the seeded `work-on-task` and `review-offered-task` skills from `templates/skills/` via `client.beta.skills.create` (one-shot per skill content hash; the CLI dedupes against an existing inventory).
...
Skills are **not** synced into a filesystem path on the worker (the worker doesn't run the model). They're uploaded once during `claude-managed-setup` via `client.beta.skills.create({ content, name, ... })` and referenced by ID on the Agent. The remaining command skills come from `plugin/commands/*.md`. The runner turn skills, `work-on-task` and `review-offered-task`, come from `templates/skills/<name>/config.json` and `content.md`, rendered with the same frontmatter builder used by the seeder. Local workers receive that pair through DB-to-filesystem skill sync; managed agents receive the uploaded pair. Both sources remain in the repo.
