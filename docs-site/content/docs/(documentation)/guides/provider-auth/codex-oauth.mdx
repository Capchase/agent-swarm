...
Migration 068 (`src/be/migrations/068_codex_oauth_pool.sql`) added `api_key_status` rows for `CODEX_OAUTH` keys, enabling the same rate-limit-aware pool logic already used for Claude OAuth tokens.
