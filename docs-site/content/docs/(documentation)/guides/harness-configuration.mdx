...
Claude Code is the default harness.
It supports two transports: CLI, the default, and the optional Claude Agent SDK.
Both transports start the installed Claude Code executable as a child process.

Select **Claude transport** beneath **Harness** on the agent details page.
Choose **CLI**, **SDK**, or **Inherit** to use the configured default.
Saving affects future sessions. An active session retains its transport.
Selecting another harness hides this control and preserves the saved Claude setting.

`CLAUDE_TRANSPORT=cli|sdk` uses the existing configuration precedence: repository, agent, global, deployment environment, then `cli`.
Repository settings apply when the task supplies a registered repository.
Set the global default under **Settings > Configuration**.
The SDK supports the same OAuth and API-key credential selection as CLI, including credential pools.
When both credential types exist, Swarm preserves both selected values and prefers OAuth for validation and tracking.
Claude Code determines which credential it uses.

SDK sessions retain Swarm context preambles and never use native session resume.
They share settings, project instructions, plugins, hooks, strict MCP configuration, telemetry, and summary preparation with CLI sessions.
`CLAUDE_BINARY` selects the executable and any wrapper prefix.
A custom wrapper must support the SDK protocol. An executable override alone does not establish compatibility.
The SDK preserves custom argument order and executable prefixes.
It rejects arguments that replace protocol framing, MCP isolation, the appended Swarm prompt, or native continuation behavior.
`CLAUDE_QUEUE_STEERING=0` disables queued input for either transport.
The worker rejects SDK selection when a bridge is effective, including legacy bridge binaries.
...
| `CLAUDE_TRANSPORT` | No | `cli` | Claude execution transport: `cli` or `sdk`. Applies to future sessions |
...
The Fable weekly limit arrives as `rateLimitType: "seven_day_overage_included"`. Claude Code CLI emits three model-scoped weekly windows on the same `rate_limit_event`, alongside the key-wide `five_hour`, `seven_day`, and `overage` windows:
...
A rejection on one of these three windows blocks only that model family on the key — the key's `status` stays `available` for every other model, unlike a key-wide rejection (`five_hour` / `seven_day` / `overage`) which marks the whole key `rate_limited`. `GET /api/keys/status` exposes the derived, readable view as `modelLimits` per key (model, window, reset time, whether it's still active).

Before picking a key, the worker resolves the task's model (or `modelTier`) and sends the family on `GET /api/keys/available?model=<family>` (`fable`, `opus`, or `sonnet` — Haiku has no weekly window in the CLI enum, so a Haiku task sends no `model` param). The server excludes a key with an active rejected window for that family from the pick.

When every key in a pool is either key-wide rate-limited or blocked by the requested model's window, `MODEL_WINDOW_EXHAUSTED_POLICY` controls what the worker does:

- `fail` (default) — fail the task immediately with the model name and earliest reset time. Re-dispatch with another model or `modelTier`, or wait.
- `fallback` — legacy behavior: pick a key at random anyway.

The policy applies at task admission only. Worker boot, credential recovery, and periodic configuration refresh never fail on an exhausted window, so a worker whose default model (`MODEL_OVERRIDE`) is exhausted still starts and still accepts tasks for another model.

Windows are independent. A model-scoped rejection never clears a key-wide rejection from the same session, and the reverse is also true: the worker reports both. Only a later non-rejected event for the same window clears a rejection. Stored window snapshots merge per window, and a snapshot never replaces one that was observed later (`lastSeenAt`), so an older `allowed` read cannot reopen an exhausted window. A structured rejection keeps the time its event was observed, so a rejection reported late cannot overwrite a recovery that another worker saw after it. Only a text-only rejection, which has no event, takes the report time.
...
### ACP: "ACP target failed during startup"
...
### ACP: agent never calls swarm MCP tools
...
### Identifying the Claude transport

The agent list and task detail page label Claude SDK transport separately from the CLI transport. Task details use the recorded session transport, so changing an agent’s setting does not relabel its past sessions.
