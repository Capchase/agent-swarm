...
| `agentswarm.db.retention.rows_deleted` | Counter | `{row}` | once per table attempt | Rows deleted. Always 0 when `dry_run:true`. |
...
| `agentswarm.db.retention.slowest_statement_ms` | Gauge | `ms` | once per table attempt | Slowest single DELETE in the slice. The event-loop stall signal. |
