...
|--------|------|-------------|
| `agentswarm.session.cost.usd` | `{usd}` | USD cost per finalized session-cost record. Not emitted for zero-cost sessions. |
| `agentswarm.session.tokens` | `{token}` | Token count per finalized session-cost record, split by `token_type`. Not emitted for zero-count classes. |
...
`agentswarm.session.tokens` additionally carries:
...
| Panel | Metric query |
|-------|-------------|
| Total cost by harness | `sum(agentswarm.session.cost.usd)`, group by `harness` |
| Cost by model | `sum(agentswarm.session.cost.usd)`, group by `model` |
| Cost by pricing source | `sum(agentswarm.session.cost.usd)`, group by `cost_source` |
| Input tokens by harness | `sum(agentswarm.session.tokens)` where `token_type = 'input'`, group by `harness` |
| Output tokens by model | `sum(agentswarm.session.tokens)` where `token_type = 'output'`, group by `model` |
| Cache efficiency | `sum(agentswarm.session.tokens)` where `token_type = 'cacheRead'` ÷ `token_type = 'input'` |
| Error session cost | `sum(agentswarm.session.cost.usd)` where `is_error = true` |
...
Use the `agentswarm.session.cost.usd` OTLP metric counter for cost aggregations and dashboards. For per-span cost attribution in trace queries, filter on the `agentswarm.cost.total_usd` span attribute on `worker.session` spans:
...
| Panel | Signal |
|-------|--------|
| Cost by harness | `sum(agentswarm.session.cost.usd)` metric, grouped by `harness` |
| Cost by model | `sum(agentswarm.session.cost.usd)` metric, grouped by `model` |
| Token usage by type | `sum(agentswarm.session.tokens)` metric, grouped by `token_type` |
| Cache efficiency | `sum(agentswarm.session.tokens)` where `token_type=cacheRead` ÷ `input` |
