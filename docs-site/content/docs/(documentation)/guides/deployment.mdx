...
- `DB_RETENTION_TICK_BUDGET_MS` (default `30000`) — wall-clock budget for one sweep tick, shared evenly across enabled tables.
- `DB_RETENTION_CATCHUP_INTERVAL_MS` (default `60000`) — delay before the next tick when a table is still undrained. The normal cadence stays hourly once every enabled table is drained.
- `DB_RETENTION_MAX_STATEMENT_MS` (default `250`) — target ceiling for one DELETE statement; the adaptive batch sizer tunes its batch size against this.
