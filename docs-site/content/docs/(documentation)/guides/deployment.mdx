...
For Kubernetes, see [Kubernetes and Helm](/docs/guides/kubernetes) for installation, API ingress, dashboard access, and CORS diagnostics.
...
### Step 6: HTTPS with Caddy (optional, needed for the hosted dashboard)

The dashboard at [app.agent-swarm.dev](https://app.agent-swarm.dev) is served over HTTPS, so browsers block a plain-HTTP API. OAuth callbacks, webhooks, and page links also need a public HTTPS origin. The Compose example ships an optional Caddy reverse proxy that obtains Let's Encrypt certificates automatically.

1. Point DNS for an API domain and an agent-fs domain at this host, and open ports 80 and 443.
2. Add to `.env`:
...
Caddy proxies the API domain to `api:3013` and the agent-fs domain to `agent-fs:7433`. Once it runs, stop exposing ports 3013 and 7433 on a public interface (firewall them or remove the `ports` entries). Certificates persist in the `caddy_data` volume. Requires Docker Compose v2.23.1 or newer for the inline Caddyfile.

If you already run a reverse proxy (Caddy, nginx, Traefik, Cloudflare Tunnel), skip the profile and set only `PUBLIC_MCP_BASE_URL` to the HTTPS origin that proxy serves. When self-hosting the dashboard on your own domain, also set `CORS_ALLOWED_ORIGINS` to that origin.
