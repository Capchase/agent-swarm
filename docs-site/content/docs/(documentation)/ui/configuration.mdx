...
| Harness & tools | `CLAUDE_TRUST_PRESEED`, `SCRIPTS_ONLY_MCP`, `MULTI_RUNTIME_ENABLED`, `CAPABILITIES`, `WORKER_API_READY_TIMEOUT_SECONDS` |
...
`CLAUDE_TRUST_PRESEED` is on by default. It writes Claude Code's trust marker for the worker workspace, every registered repository clone path, and the task directory. Set it to `false` to disable this pre-seed for later sessions.
