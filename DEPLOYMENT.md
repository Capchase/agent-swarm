...
4. Add required scopes: `app_mentions:read`, `assistant:write`, `channels:history`, `channels:manage`, `channels:read`, `chat:write`, `chat:write.customize`, `chat:write.public`, `commands`, `files:read`, `files:write`, `groups:history`, `groups:read`, `groups:write`, `im:history`, `im:read`, `im:write`, `mpim:history`, `mpim:read`, `mpim:write`, `reactions:read`, `reactions:write`, `users:read`
   After changing scopes in `slack-manifest.json`, apply the updated manifest to the Slack app and reinstall the app to the workspace for the changes to take effect. `reactions:read` in particular is required for the acceptance-reaction cleanup in `src/slack/ack.ts` to read the message's current reactions (`reactions.get`); an app installed before this scope was added must be reinstalled, or `reactions.get` fails with `missing_scope` and cleanup silently falls back to the configured reaction names only.
